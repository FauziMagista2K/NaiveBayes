...
<body>
	...
	include 'koneksi.php';
	$kode_info = $_GET['kode_info'];
	$sql = mysqli_query($db,"select * from tb_info where kode_info='$kode_info'");
	while($data = mysqli_fetch_array($sql)){
		...
		<form method="post" action="">
                                        ...
                                        <div class="row">
                                            ...
                                            <input type="hidden" name="kode_info" value="<?php echo $data['kode_info']; ?>">
                                         <textarea  class="form-control" id="nama_info" name="nama_info"><?php echo $data['nama_info'] ?></textarea>       
                                        </div>
                                            </div>
                                              <center>  
                                              <a href="info.php" name="submit" class="btn alazea-btn mt-15">Kembali</a>
...
$kode_info = $_POST['kode_info'];
$nama_info = $_POST['nama_info'];

 
// update data ke database
$sql = mysqli_query($db,"update tb_info set nama_info='$nama_info' where kode_info='$kode_info'")or die(mysqli_error($db));
 
  
if($sql){
    echo '<script>alert("Berhasil menyimpan data."); document.location="info.php?kode_info='.$kode_info.'";</script>';
  }else{
